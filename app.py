...
def check_for_completed_boxes(x, y, player):
    ...
    if y < GRID_SIZE:
        if all([game_state["horizontal_lines"][y][x],
                game_state["horizontal_lines"][y + 1][x] if y + 1 < GRID_SIZE + 1 else 0,
                game_state["vertical_lines"][x][y],
                game_state["vertical_lines"][x + 1][y] if x + 1 < GRID_SIZE else 0]):
    ...
    if y > 0:
        if all([game_state["horizontal_lines"][y - 1][x],
                game_state["horizontal_lines"][y][x],
                game_state["vertical_lines"][x][y - 1],
                game_state["vertical_lines"][x + 1][y - 1] if x + 1 < GRID_SIZE else 0]):
            ...
            if game_state["turn"] == player_id:
                ...
                if orientation == "horizontal":
                    if game_state["horizontal_lines"][y][x] == 0:
                        game_state["horizontal_lines"][y][x] = player_id  # Armazena o jogador que desenhou a linha
                        completed_box = check_for_completed_boxes(x, y, player_id)
                elif orientation == "vertical":
                    if game_state["vertical_lines"][y][x] == 0:
                        game_state["vertical_lines"][y][x] = player_id  # Armazena o jogador que desenhou a linha
                        completed_box = check_for_completed_boxes(x, y, player_id)
