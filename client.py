        ...
        if self.game_state:
            ...
            for i, row in enumerate(self.game_state["horizontal_lines"]):
                for j, line in enumerate(row):
                    if line:  # Se a linha foi desenhada
                        # Verifica o dono da linha (quem desenhou a linha)
                        color = "blue" if self.game_state["horizontal_lines"][i][j] == "player1" else "red"
            ...
            for i, row in enumerate(self.game_state["vertical_lines"]):
                for j, line in enumerate(row):
                    if line:  # Se a linha foi desenhada
                        # Verifica o dono da linha (quem desenhou a linha)
                        color = "blue" if self.game_state["vertical_lines"][i][j] == "player1" else "red"
                        x1 = j * self.cell_size + 15
                        y1 = i * self.cell_size + 15
                        x2 = x1
                        y2 = (i + 1) * self.cell_size + 5
                        self.canvas.create_line(x1, y1, x2, y2, fill=color, width=2)
            ...
            for i, row in enumerate(self.game_state["boxes"]):
                for j, owner in enumerate(row):
                    if owner:
                        # Determina a cor da caixa com base no jogador que a possui
